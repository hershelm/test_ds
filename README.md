...
1. create an environment `test_ds` with the help of [conda],
   ```
...
2. Create concrete dependencies as `environment.lock.yaml` for the exact reproduction of your
   environment with:
   ```
   conda env export -n test_ds -f environment.lock.yaml
   ```
...
├── src
│   └── test_ds             <- Actual Python package where the main functionality goes.
